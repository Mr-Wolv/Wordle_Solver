...
plus the exact documented residual words (so the contract is anchored even
on a quick run). A separate exhaustive test (``-m exhaustive``) replays all
2,315 official answers in both modes — this is the definitive regression
gate the solver's accuracy claims rest on.

Mode separation (see ReadME "Performance Benchmarks"):
  * Hinted modes (normal+hint, hard+hint) are the protected baseline: 100%
    solve rate, and their average turns must never regress.
  * No-hint modes carry the residual gap. As of this change:
      - NORMAL no-hint: 100% (every official answer solved in <=6).
      - HARD   no-hint: 2309/2315 = 99.74%. The 6 remaining failures are tight
        4-letter-shared sibling clusters (-atch, -ound, -oxer/-oyer) that
        cannot be split under NYT *hard* rules (every legal guess must be a
        pool word, and any word inside such a cluster yields the identical
        pattern to all its siblings, so it can only eliminate itself ->
        one peel per turn -> 7 guesses needed). They are provably
        uncloseable in hard no-hint; documented here as the known ceiling.
"""
import pytest
...
# No-hint NORMAL residuals: NONE — normal no-hint now solves 100%.
NORMAL_RESIDUALS: list[str] = []
# No-hint HARD residuals: the 6 provably hard-uncloseable tight clusters.
HARD_RESIDUALS = ["FOYER", "HATCH", "HOUND", "HUNCH", "LATCH", "MOUND"]
...
NO_HINT_FLOOR = {
    ...
    True: 2309,    # HARD   no-hint must stay >= the hard-mode ceiling
}
...
def test_exhaustive_contract():
    ...
    assert hard_failures == set(HARD_RESIDUALS), (
        f"hard residuals drifted: {hard_failures ^ set(HARD_RESIDUALS)}"
    )
